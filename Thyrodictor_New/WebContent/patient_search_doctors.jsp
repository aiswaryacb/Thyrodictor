  ...
  <body>    
   ...
   <section id="service">
      <div class="container">
        <div class="row">
          <div class="col-lg-12 col-md-12">
               <form class="appointment-form"  id="search" name="search" method="post" action="patient_search_doctors.jsp"">
                    <div class="row">
                      	<div class="col-md-1 col-sm-1"></div>
                        <div class="col-md-2 col-sm-2">
                          <label for="email" class="control-label">Name <span class="required">*</span>
                          </label>
                        </div>
                        <div class="col-md-6 col-sm-6">
                          <input type="text" id="name" name="name" class="wp-form-control wpcf7-email" placeholder="Name">
                            	<p id="error"></p>  
                        </div>
                        <div class="col-md-2 col-sm-2"> 
                           	<button class="wpcf7-submit button--itzel" type="submit"><i class="button__icon glyphicon glyphicon-search"></i><span>Search</span></button>  
                        </div>
                        <div class="col-md-1 col-sm-1"></div>
                     </div> 
        ...
        <div class="row">
        <%
		 ResultSet rs1= stmt.executeQuery("select * from doctor_details where (doctor_name='"+sname+"' or doctor_name like '%"+sname+"%') and doctor_status='accepted' order by doctor_name;");

        if(rs1.next()){
        	float rating=rs1.getFloat(9);
        	float nrates=rs1.getFloat(10);
        	float rate=0;
        	if(nrates>0){
        		rate=rating/nrates;
        	}
        	
        	//String r=String.format("%.1f", Float.torate);
        	int slno=1;
        	%>
        	<div class="col-lg-12 col-md-12">
        	<table class="table">
        	...
        	<tr>
        		...
        		<td><%=rs1.getString(3) %>&nbsp;&nbsp;<%= df2.format(rate)%><span class="fa fa-star checked"></span></td>
        		<td><%=rs1.getString(5) %></td>
        		<td><%=rs1.getString(4) %></td>
        		<td><%=rs1.getString(7) %></td>
        		<td><a href="patient_doc_hos_details.jsp?doc_id=<%=rs1.getString(1) %>">View</a></td>
        		<td>
        		<form action="patient_rating.jsp">
        		<input type="hidden" id="doc" name="doc" value="<%=rs1.getString(1)%>">
					<input id="input-1" name="input-1" class="rating rating-loading" data-min="0" data-max="5" data-size="s" data-step="0.5" data-show-clear="false" data-show-caption="false" onchange="this.form.submit()">
				</form>
        		</td>
        	</tr>
        	<%
        	while(rs1.next()){
        		rating=rs1.getFloat(9);
            	nrates=rs1.getFloat(10);
            	rate=0;
            	if(nrates>0){
            		rate=rating/nrates;
            	}
            	//formatter.format("%.1f", rate);
        		...
        		<form action="patient_rating.jsp">
        		<input type="hidden" id="doc" name="doc" value="<%=rs1.getString(1)%>">
					<input id="input-1" name="input-1" class="rating rating-loading" data-min="0" data-max="5" data-step="0.5" data-size="s" data-show-clear="false" data-show-caption="false" onchange="this.form.submit()">
				</form>
        		</td>
        	</tr>
        		<%
