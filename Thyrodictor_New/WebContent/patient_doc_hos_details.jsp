	...
	if(!con.isClosed()){
		stmt=con.createStatement();
		 ...
		 String patient_id=rs.getString(1);
			...
			ResultSet rs2= stmt.executeQuery("select doctor_name from doctor_details where doctor_id='"+doc_id+"'");
			rs2.next();
...
<html lang="en">
  ...
  <body>    
            ...
            <div class="container">
              ...
              <div class="blog-breadcrumbs-right">
                <ol class="breadcrumb">
                  ...
                  <li><a href="patient_search_doctors.jsp?name=">Doctors</a></li>
                  <li class="active">Doctor's Hospital Details</li>
                </ol>
              </div>
            </div>
          </div>
   ...
   <section id="service">
      ...
      <div class="row">
	      <div class="col-lg-12 col-md-12">
	      	<center><h3 style="color: blue;">Hospital Details of: Dr. <%=rs2.getString(1) %></h3></center>
	      </div>
      </div>
        <div class="row">
        <%
		 ResultSet rs1= stmt.executeQuery("select * from hospital_details where doctor_id='"+doc_id+"';");

        if(rs1.next()){
        	String placeid=rs1.getString(4);
        	String place="";
        	for(i=0;i<n;i++){
        	...
        	<tr>
        		...
        		<td><%=place %></td>
        		<td><%=rs1.getString(5) %> to <%=rs1.getString(6) %></td>
        		<td><%=rs1.getString(7) %></td>
        		<td><%=rs1.getString(8) %></td>
        	</tr>
        	<%
        	while(rs1.next()){
        		placeid=rs1.getString(4);
            	place="";
            	for(i=0;i<n;i++){
            		if(dist_id[i].equals(placeid)){
            			place=dist_name[i];
        	...
        	<center><h4><a href="patient_search_doctors.jsp?name="><span class="glyphicon glyphicon-menu-left"></span>Back</a></h4></center>
        	</div>
        	<%
        }else{
        	%>
        	<div class="col-lg-12 col-md-12">
        	<center><h3>No hospital details to show !</h3></center>
        	</div>
        	<%
        	}
        %>
